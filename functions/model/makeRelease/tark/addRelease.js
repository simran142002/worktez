...
exports.addRelease = function(request, response) {
  ...
  const ownerName = request.body.data.OwnerName;
  const repoName = request.body.data.RepoName;
  ...
  const promise = getAllReleaseData(orgDomain).then((data) => {
    ...
    setRelease(description, releaseName, tagName, targetBranch, ifDraft, preRelease, generateRelease, ownerName, repoName, teamId, releaseId, orgDomain, releaseDate).catch(
        (error) => {
          result = {data: error};
          status = 500;
