...
export class EditReleaseComponent implements OnInit {
  ...
  constructor(private validationService:ValidationService, private backendService: BackendService, public functions: AngularFireFunctions, public errorHandlerService: ErrorHandlerService, private httpService: HttpServiceService) { }
    ...
    this.httpService.getReleaseDetails().subscribe((data) => {
      for(let i in data){
        if(data[i].tag_name==this.tagName){
          ...
          const bearerToken = "";
          this.httpService.updateGithubRelease(release_Id, bearerToken, this.tagName, this.targetBranch, this.releaseName, this.description, this.response1, this.response2, this.response3);
